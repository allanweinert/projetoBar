...
@Component({
  ...
  providers: [MovimentacaoEstoquePesquisaService],
})
export class MovimentacaoEstoquePesquisaComponent implements OnInit {
  ...
  pesquisar() {
    ...
    this.movimentaEstoquePesquisaService
  ...
  getDescricaoTipo(tipoValor: string): string {
    const tipo = this.movimentaEstoquePesquisaService
      .listarMovimentacoes()
      .find((tipo) => tipo.value === tipoValor);
    return tipo.label;
  }
}
